...
const createpostsaved = async (req, res) => {
    try {
        ...
        const query = 'INSERT INTO daily_posts_saved (id, post_id, user_id, saved_date) VALUES ($1, $2, $3, $4) RETURNING *';
...
const updatepostsaved = async (req, res) => {
    try {
        ...
        const query = 'UPDATE daily_posts_saved SET post_id = $1, user_id = $2, saved_date = $3 WHERE id = $4 RETURNING *';
...
const deletepostsaved = async (req, res) => {
    try {
        ...
        const query = 'DELETE FROM daily_posts_saved WHERE id = $1';
        const values = [id];
...
const getpostsaved = async (req, res) => {
    try {
        const query = 'SELECT * FROM daily_posts_saved ORDER BY id ASC';
